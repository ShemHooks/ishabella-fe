...
import { TrendingUp, ShoppingCart, Package, AlertTriangle, ChevronRight } from 'lucide-react';
...
export default function CleanDashboard() {
  ...
  return (
    <div className="p-8 bg-slate-50 min-h-screen">
      {/* 1. WELCOME SECTION */}
      <div className="mb-10">
        <h2 className="text-3xl font-black   uppercase tracking-tighter">Good Day Admin!</h2>
        ...
        {stats.map((stat, index) => (
          ...
          >
            ...
            <div>
              ...
              <p
                className={`text-3xl font-black ${stat.title === 'Low Stock Alert' ? 'text-red-500' : 'text-[#064e3b]'}`}
              >
                {stat.value}
              </p>
              <p className="text-xs text-slate-400 mt-2 font-medium italic">{stat.description}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[#064e3b] p-8 rounded-[40px] text-white flex flex-col justify-center shadow-xl shadow-emerald-900/10">
          ...
          <p className="text-emerald-100 text-sm leading-relaxed ">
            "Your store is currently live and customers can browse your solar panels and air units
            without any issues."
          </p>
        </div>
      </div>
